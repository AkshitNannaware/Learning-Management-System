...
// ─────────────────────────────────────────────
// SIMPLE CREATE COURSE FORM (YouTube Link + Essential Fields)
// ─────────────────────────────────────────────
function SimpleCreateCourseForm({ onBack, onCreate, loading: parentLoading }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const youtubeRegex = /^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.be)\/watch\?v=|(youtu\.be\/)/
          ...
          <div className="flex flex-col items-start justify-between gap-4 lg:flex-row">
            <div>
              <div className="inline-flex items-center gap-2 rounded-[12px] bg-[#ffd966] px-3 py-1.5 mb-4">
                ...
                <span className="text-[12px] font-medium text-[#4b2e00]">Admin Workspace</span>
              </div>
              <h1 className="text-[26px] font-bold leading-tight text-[#0f172a] sm:text-[32px]">Course Management</h1>
              <p className="mt-2 max-w-[560px] text-[13px] text-[#94a3b8]">
                Add courses by pasting YouTube links. Manage, edit, and track all your courses in one place.
              </p>
            ...
            <div key={course._id} className="bg-white border border-black/[0.08] rounded-lg overflow-hidden hover:shadow-md transition-shadow">
              ...
              <div className="p-4">
                ...
                <div className="flex flex-wrap gap-3 mb-3 text-[11px] text-[#94a3b8]">
                  <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" />{new Date(course.createdAt).toLocaleDateString()}</span>
